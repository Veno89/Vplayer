...
pub fn validate_playlist_name(name: &str) -> AppResult<String> {
    ...
    if trimmed.chars().count() > 255 {
        return Err(AppError::Validation(
            "Playlist name too long (max 255 characters)".to_string(),
        ));
    }

    if trimmed.chars().any(|c| c.is_control()) {
        return Err(AppError::Validation(
            "Playlist name contains invalid control characters".to_string(),
        ));
    }
...
mod tests {
    ...
    fn test_validate_playlist_name_valid() {
        ...
        assert_eq!(
            validate_playlist_name("  Chill / Focus  ").unwrap(),
            "Chill / Focus"
        );
    }
