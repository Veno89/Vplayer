...
pub fn get_filtered_tracks(
    filter: crate::database::TrackFilter,
...
pub fn set_track_rating(
    ...
    state
        .db
        .set_track_rating(&track_id, validated_rating)
        .map_err(|e| AppError::Database(e.to_string()))
}
...
pub fn update_track_path(
    ...
    state
        .db
        .update_track_path(&track_id, &new_path)
        .map_err(|e| AppError::Database(e.to_string()))
}
...
pub fn get_album_art_batch(
    ...
    if track_ids.len() > 128 {
        return Err(AppError::Validation(
            "Album-art batches are limited to 128 tracks".to_string(),
        ));
    }
...
#[tauri::command]
pub fn extract_and_cache_album_art(
    track_id: String,
    track_path: String,
    state: tauri::State<'_, AppState>,
) -> AppResult<Option<String>> {
    ...
    match Scanner::extract_album_art(&track_path) {
        Ok(Some(art_data)) => {
            if art_data.len() > 16 * 1024 * 1024 {
                return Err(AppError::Validation(
                    "Embedded album art exceeds the 16 MiB safety limit".to_string(),
                ));
            }
            // Cache in database
            state
                .db
                .set_album_art(&track_id, &art_data)
        ...
        Ok(None) => Ok(None),
        Err(e) => Err(AppError::Scanner(format!(
...
pub fn update_track_tags(
    ...
    let track_num_i32 = tags
        .track_number
        .as_ref()
        .and_then(|t| t.parse::<i32>().ok());
    let disc_num_i32 = tags
        .disc_number
        .as_ref()
