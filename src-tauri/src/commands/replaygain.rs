...
#[tauri::command]
pub async fn analyze_replaygain(
    track_id: String,
    track_path: String,
    state: tauri::State<'_, AppState>,
) -> AppResult<ReplayGainData> {
    let track_path =
    ...
    let data = tauri::async_runtime::spawn_blocking(move || analyze_track(&path))
        .await
...
#[tauri::command]
pub fn get_track_replaygain(
    track_id: String,
    track_path: String,
    state: tauri::State<'_, AppState>,
) -> AppResult<Option<ReplayGainData>> {
    let track_path =
        super::path_authority::authorize_track_path(&state.db, &track_id, &track_path)?;
