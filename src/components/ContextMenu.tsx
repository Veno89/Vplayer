...
export type MenuItem = MenuItemAction | MenuSeparator;
...
interface TrackContextMenuParams {
  track: Track;
  onPlay: () => void;
  onAddToQueue: () => void;
  ...
  onShowInfo?: () => void;
  onSetRating?: () => void;
  currentTrack?: number | boolean | null;
  isPlaylist?: boolean;
}

interface PlaylistForContextMenu {
  trackCount: number;
  [key: string]: unknown;
}

interface PlaylistContextMenuParams {
  playlist: PlaylistForContextMenu;
  onPlay: () => void;
  onRename: () => void;
  onDuplicate: () => void;
  onExport: () => void;
...
interface FolderForContextMenu {
  path: string;
  [key: string]: unknown;
}

interface FolderContextMenuParams {
...
export function getTrackContextMenuItems({
  ...
  currentTrack,
  isPlaylist = false,
}: TrackContextMenuParams): MenuItem[] {
  ...
  return items.filter(Boolean) as MenuItem[]; // Filter out false/null values
}
...
export function getPlaylistContextMenuItems({
  ...
  onDelete,
  isActive,
}: PlaylistContextMenuParams): MenuItem[] {
  return [
    {
      icon: Play,
      label: 'Play Playlist',
...
export function getFolderContextMenuItems({
  ...
  onRemove,
  onShowInExplorer,
}: FolderContextMenuParams): MenuItem[] {
  return [
    {
      icon: Music,
      label: 'Rescan Folder',
