...
export function LibraryStatsWindow() {
  ...
  const localStats = React.useMemo(() => {
    ...
    const years = tracks.map(t => parseInt(t.year)).filter(y => !isNaN(y) && y > 1900);
    const formats = tracks.reduce((acc, t) => {
    ...
    const neverPlayed = tracks.filter(t => !t.play_count || t.play_count === 0).length;
    
    // Rating stats
    const rated = tracks.filter(t => t.rating && t.rating > 0);
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <StatSection title="Playback Stats" icon={Clock}>
          ...
          {/* Most Played */}
          {localStats?.mostPlayed?.length > 0 && (
            <div className="mt-4">
              <p className="text-slate-400 text-sm mb-2">Most Played</p>
              <div className="space-y-1">
                {localStats.mostPlayed.map((track, i) => (
