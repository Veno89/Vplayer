...
export default function ThemeEditorWindow() {
  ...
  const handleSaveTheme = () => {
    ...
    const newTheme = {
      ...
      backgroundSecondary: colors.bg2,
      backgroundTertiary: colors.bg3,
      text: colors.text,
  ...
  const handleApplyTheme = (theme: ColorScheme) => {
    setColors({
      primary: theme.primary || '#3b82f6',
      accent: theme.accent || '#60a5fa',
      bg1: theme.background || '#0f172a',
      bg2: theme.backgroundSecondary || '#1e293b',
      bg3: theme.backgroundTertiary || '#334155',
      text: theme.text || '#ffffff',
      textSecondary: theme.textMuted || '#94a3b8',
      border: theme.border || '#475569'
    });
              ...
              {Object.entries(colorLabels).map(([key, label]) => (
                <div key={key} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-white/50 font-mono">{colors[key as keyof ThemeColors]}</div>
                  </div>
                </div>
              ))}
            </div>
