...
export default function LyricsWindow() {
  ...
  const currentTrackIndex = useStore((state) => state.currentTrack);
  const progress = useStore((state) => state.progress);
  const { playbackTracks } = usePlayerContext();
  const currentTrack = currentTrackIndex !== null ? playbackTracks[currentTrackIndex] ?? null : null;
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {lyrics && !loading && !error && (
            <div className="space-y-4">
              {lyrics.lines.map((line: LrcLine, index: number) => {
                const isCurrent = index === currentLine;
                const isNext = currentLine !== null && index === currentLine + 1;
