      ...
      <div className="w-full max-w-2xl mx-4">
        ...
        <div className="bg-slate-800/50 backdrop-blur-lg rounded-2xl border border-slate-700/50 overflow-hidden">
          ...
          <div className="p-8 text-center border-b border-slate-700/50">
            <div className={`inline-flex p-4 rounded-2xl bg-linear-to-br from-cyan-500/20 to-violet-500/20 mb-4`}>
          ...
          <div className="px-8 pb-8 flex items-center justify-between">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={handleNext}
                className="px-6 py-2.5 bg-linear-to-r from-cyan-600 to-violet-600 hover:from-cyan-500 hover:to-violet-500 text-white rounded-xl transition-all flex items-center gap-2 font-medium"
              >
...
function LibraryStep({ onAddFolder, folderAdded, selectedFolder }: { onAddFolder: () => void; folderAdded: boolean; selectedFolder: string | null }) {
  return (
    ...
    <div className="space-y-3">
      {shortcuts.map((shortcut, i) => (
        <div key={i} className="flex items-center justify-between p-3 bg-slate-700/30 rounded-lg">
          ...
          <kbd className="px-3 py-1 bg-slate-800 text-cyan-400 text-sm font-mono rounded-sm border border-slate-600">
            {shortcut.key}
          </kbd>
        </div>
      ))}
      <p className="text-slate-500 text-sm text-center mt-4">
        Press <kbd className="px-2 py-0.5 bg-slate-800 rounded-sm text-xs">?</kbd> anytime to see all shortcuts
      </p>
    </div>
  );
}
...
function CompleteStep() {
  return (
    <div className="text-center space-y-6">
      <div className="inline-flex p-6 rounded-full bg-linear-to-br from-emerald-500/20 to-cyan-500/20">
      ...
      <div className="p-4 bg-slate-700/30 rounded-xl inline-block">
        <p className="text-slate-400 text-sm">
          Need help? Press <kbd className="px-2 py-0.5 bg-slate-800 rounded-sm text-xs mx-1">?</kbd> to view all shortcuts
        </p>
      </div>
    </div>
  );
...
function FeaturePreview({ icon: Icon, title, description, color }: { icon: React.ComponentType<{ className?: string }>; title: string; description: string; color: 'cyan' | 'violet' | 'pink' }) {
  const colors: Record<'cyan' | 'violet' | 'pink', string> = {
  ...
  return (
    <div className={`p-4 rounded-xl bg-linear-to-br ${colors[color]}`}>
