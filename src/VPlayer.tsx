...
const VPlayerInner = () => {
  ...
  const toggleWindowWithResize = useCallback((windowId: string) => {
  ...
  useShortcuts({
    ...
    toggleRepeat: () => {
      ...
      setRepeatMode(r === 'off' ? 'all' : r === 'all' ? 'one' : 'off');
    },
    toggleWindow: toggleWindowWithResize,
    focusSearch: () => (document.querySelector('input[type="text"][placeholder*="Search"]') as HTMLElement | null)?.focus(),
    audio,
  });
